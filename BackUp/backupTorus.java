...
{	
	...
	public final static class TorusRenderPanel extends GLRenderPanel
	{
		...
		public void init(RenderContext r)
		{
			...
			float v3d[][] = new float[torWidth*torHeight+addPoints][dim3];
			float v[] = new float[3*(torWidth*torHeight+addPoints)];
			float c[] = new float[3*(torWidth*torHeight+addPoints)];
			int[] indices = new int[((torWidth-1)*(torHeight-1)) * 6 + torWidth * tri * faces + torWidth*(torHeight - 1)];
			int k = 0;
			//System.out.print("TestOutput:" + indices.length);
			...
			for(int row = 0; row < torHeight; row++)
			{
				for(int column = 0; column < torWidth; column++)
				{
					...
					c[column*dim3+row*torWidth*dim3 + 1] = 1;
					c[column*dim3+row*torWidth*dim3 + 2] = row%3;
					if(row < torHeight - 1)
					{
...
/*						System.out.printf("col:\t%d\trow:\t%d\t", column, row);
						System.out.printf("ind: %d %d %d\t", 
								column + row * torWidth, 
								(column + 1)%torWidth + row * torWidth, 
								(column + 1)%torWidth + (row + 1)*torWidth);
						System.out.printf("ind: %d %d %d\n", 
								column + row * torWidth, 
								(column + 1)%torWidth + (row + 1)*torWidth, 
								column + row * torWidth + torWidth);*/
					}
					else
					{
						indices[k++] = column + row * torWidth;
