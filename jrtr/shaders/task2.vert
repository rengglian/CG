...
{		
	...
	for(int Loop = 0; Loop < nLights; Loop++)
	{
		...
		cd.x += cl.x*materialSpecular.x*pow(max(dot(R,E),0.0f),materialShininess);
		cd.y += cl.y*materialSpecular.y*pow(max(dot(R,E),0.0f),materialShininess);
		cd.z += cl.z*materialSpecular.z*pow(max(dot(R,E),0.0f),materialShininess);
