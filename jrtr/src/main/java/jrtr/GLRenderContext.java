...
public class GLRenderContext implements RenderContext {
	...
	private void setMaterial(Material m) {
		...
		if(m != null && m.shader != null) {
			...
			if(m.map != null){
				gl.glActiveTexture(GL3.GL_TEXTURE1);	// Work with texture unit 0
				gl.glEnable(GL3.GL_TEXTURE_2D);
				gl.glBindTexture(GL3.GL_TEXTURE_2D, ((GLTexture)m.map).getId());
				gl.glTexParameteri(GL3.GL_TEXTURE_2D, GL3.GL_TEXTURE_MAG_FILTER, GL3.GL_LINEAR);
				gl.glTexParameteri(GL3.GL_TEXTURE_2D, GL3.GL_TEXTURE_MIN_FILTER, GL3.GL_LINEAR);
				// We assume the texture in the shader is called "myTexture"
				id = gl.glGetUniformLocation(activeShaderID, "myMap");
				if(id!=-1)
			...
			if(m.texture != null) {
				...
				id = gl.glGetUniformLocation(activeShaderID, "materialDiffuse");
				if(id!=-1)	
					gl.glUniform3f(	id ,m.diffuse.x, m.diffuse.y, m.diffuse.z);
				
				id = gl.glGetUniformLocation(activeShaderID, "materialSpecular");
				if(id!=-1)
					gl.glUniform3f(	id ,m.specular.x, m.specular.y, m.specular.z);
				
				id = gl.glGetUniformLocation(activeShaderID, "materialAmbient");
				if(id!=-1)
					gl.glUniform3f(	id ,m.ambient.x, m.ambient.y, m.ambient.z);
				
				id = gl.glGetUniformLocation(activeShaderID, "materialShininess");
				if(id!=-1)
					gl.glUniform1f(id ,m.shininess);
				
				if(m.experimental)
				{
					long runTime = sceneManager.getRunTime();
					id = gl.glGetUniformLocation(activeShaderID, "runTime");
					if(id!=-1)
						gl.glUniform1f(id ,runTime);					
				}
			}
			...
			if(id!=-1)
				...
				if(DEBUG)System.out.print("Could not get location of uniform variable " + lightString + "\n");
			int nLights = 1;
			...
			Vector3f e = sceneManager.getCamera().getLookAtPoint();
			gl.glUniform3f(	gl.glGetUniformLocation(activeShaderID, "e"),e.x, e.y, e.z);
			...
			if(iter != null) {
				...
				while(iter.hasNext() && nLights<8)
				{
					...
					id = gl.glGetUniformLocation(activeShaderID, lightString);
					if(id!=-1)
						gl.glUniform3f(id, l.specular.x, l.specular.y, l.specular.z);		// Set light direction
					else
						if(DEBUG)System.out.print("Could not get location of uniform variable " + lightString + "\n");
					...
					id = gl.glGetUniformLocation(activeShaderID, lightString);
					if(id!=-1)
						gl.glUniform3f(id, l.ambient.x, l.ambient.y, l.ambient.z);		// Set light direction
					else
						if(DEBUG)System.out.print("Could not get location of uniform variable " + lightString + "\n");
					...
					id = gl.glGetUniformLocation(activeShaderID, lightString);
					if(id!=-1)
						gl.glUniform4f(id, l.position.x, l.position.y, l.position.z, 0.0f);		// Set light direction
					else
						if(DEBUG)System.out.print("Could not get location of uniform variable " + lightString + "\n");
